...
MAINTAINER Wei Dong
RUN yum install -y wget yum-utils centos-release-scl tar xz glibc-static cvs subversion git unzip
...
RUN yum install -y devtoolset-4-gcc-c++ devtoolset-4-gcc-gfortran devtoolset-4-gdb devtoolset-4-elfutils devtoolset-4-oprofile ; ln -s /opt/rh/devtoolset-4/enable /etc/profile.d/devtoolset-4.sh
